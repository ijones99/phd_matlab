...
%
% [ ppAmps elCoordsX elCoordsY] = PLOT_FOOTPRINTS_SIMPLE( ...
%     elPositionXY, waveform, varargin )
% 
% arguments
%   elPositionXY: [chNum x xyInfo];
%   waveform: for template -> [samples x channels];
...
% apply to templates
templates = templates*scaleFactor*2;
waveform = waveform*scaleFactor*2;

plotXAxis = repmat(elPositionXY(:,1),1,length(xaxisScaled)) + repmat(xaxisScaled,length(elPositionXY(:,1)),1);

hold on
for i=1:size(waveform, 3);
    plotYAxis = repmat(elPositionXY(:,2),1,length(xaxisScaled))  - waveform(:,:,i);
    plot(plotXAxis' , plotYAxis',plotStyle,'Color',plotColor,'LineWidth', lineWidth );
end
