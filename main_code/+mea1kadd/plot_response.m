...
threshVal(idxCurrFile) = mea1kadd.processing.get_stim_threshold_v2(m_stim, uniqueVoltages(1:end),lookupTbStim,...
    Xread, LUT.el_name, readoutEls,'x_axis_window',[55 65],'y_thresh',-4,'do_plot', 1);

titles{idxCurrFile} = ['stim_el_',num2str(LUT.el_name(idxCurrFile))];
title(titles{idxCurrFile},'Interpreter', 'none');
figs.format_for_pub('journal_name', 'frontiers')
