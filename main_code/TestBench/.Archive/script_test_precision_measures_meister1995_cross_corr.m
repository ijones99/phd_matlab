...
precisionInfoMean = [];
precisionInfoStd = [];
precisionInfoMeanAll = [];
precisionInfoStdAll = [];
precisionInfoSigmaAll = [];
% bin data
...
xlabel('Log Jitter Sigma Value'), ylabel('Precision Ratio');
...
% plot area
for iGroup = 1:length(precisionInfo)% for all groups of repeats
    totalAreaMean = mean(precisionInfo{iGroup}.totalArea);
end

figure, plot(log(sigmaValues),totalAreaMean);
xlabel('Log Jitter Sigma Value');
ylabel('Area Under Curve');
title('Area Under Curve of Cross Correlations');
%% plot as function of percent spikes removed
errorbar(percentSpikesToRemove,precisionInfoMean_5ms,precisionInfoStd_5ms,'kx-'), hold on
title('Cross Correlation Precision Ratio As Function of % Spikes Removed, 50 repeats')

xlabel('Percent Spikes Removed'); %ylim([0 (max(precisionInfoMean_1ms)*1.5)]);
ylabel('Precision Ratio');

%% label
legend('sigma=1ms','sigma=2ms','sigma=5ms')