...
for iDir = 1:length(dirName.dataDirLong) % loop through directories
    clearvars -except dirName param_BiasIndex iDir
    cd([dirName.dataDirLong{iDir},'Matlab/']); % enter directory
    clearvars -except dirName iDir param_BiasIndex % clear vars
    script_load_data; load idxNeur ; load idxToMerge.mat; load idxFinalSel% load data
    
    load(fullfile(dirName.local.marching_sqr_over_grid, 'dataOut'))
    param_BiasIndex = [];
    param_BiasIndex.clus_num = cell2mat(dataOut.clus_no);
    param_BiasIndex.date =dirName.dataDirShort{iDir} ;
    param_BiasIndex.index = (dataOut.total_spikes_ON-dataOut.total_spikes_OFF)./...
        (dataOut.total_spikes_ON+dataOut.total_spikes_OFF);
    param_BiasIndex.total_spikes_ON = dataOut.total_spikes_ON;
    param_BiasIndex.total_spikes_OFF = dataOut.total_spikes_OFF;

    save(fullfile(dirName.local.marching_sqr_over_grid, 'param_BiasIndex.mat')...
        ,'param_BiasIndex')
end
