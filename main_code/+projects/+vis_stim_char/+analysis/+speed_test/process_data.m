...
stimFrameInfo = file.load_single_var('settings/',...
    'stimFrameInfo_Moving_Bars_Speed_Test.mat');
...
allStimChsIdx = 1:2:length(stimChangeTs{barsSpeedTestRIdx});
                ...
                for iWidth = 1:length(widthsUnique)
                    for iSpeed = 1:length(speedsUnique)
                        for iAngle = 1:length(anglesUnique)
                            % get spiketimes
                            stCurr = neurM(barsSpeedTestRIdx).ts;
                            ...
                            [spikeTrain spikeTrainSeg spikeTrainBL spikeTrainBLSeg] = spiketrains.get_raster_series(stCurr/2e4, ...
                                stimChangeTs{barsSpeedTestRIdx}/2e4, idxRaster,'baseline_sec',baselineTime );
                            
                            
                            spikeCnt = [];
                            for j =1 :length(spikeTrain)
                                
                                sweepDur_samp = diff(stimChangeTs{barsSpeedTestRIdx}(idxRaster(1,:)));
