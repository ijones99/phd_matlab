...
for iDir =4%1:length(dirNames.dataDirLong) % loop through directories
    ...
    for i=1:length(idxFinalSel.keep)
        cMap = plot.colormap_optimal_colors(length(idxFinalSel.keep));
        ...
        maxAmp(i) = max(amps);
        A(i) = 2*sqrt(maxAmp(i)/pi())/4;
        plot.circle2(ctrLoc{i}.x+jitterAmt(1), ctrLoc{i}.y+jitterAmt(2), A(i),...
            'EdgeColor',cMap(i,:), 'LineWidth', 3);
        %     ju = input(' > ')
...
for i=1:length(minMaxAmpIdx)
    plot.circle2( xLegend(minMaxAmpIdx(i)),yLegend(minMaxAmpIdx(i)), A(minMaxAmpIdx(i)),...
            'EdgeColor',cMap(minMaxAmpIdx(i),:), 'LineWidth', 3);
        
    text(  xLegend(minMaxAmpIdx(i))+10,yLegend(minMaxAmpIdx(i)), sprintf('%d',maxAmp(minMaxAmpIdx(i)))) 
