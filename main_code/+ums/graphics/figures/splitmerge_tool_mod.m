...
function execute_change(varargin)
    ...
    if length(figdata.selected) < 2
        ...
        set(h,'CurrentAxes',a)
        % colormode (arg 3): 3 = all traces, 
        % display mode:
        %   spikes.params.display.default_waveformmode = 1 for all
        %   waveforms
        %
        %   spikes.params.display.default_waveformmode = 2 for bands
        
        plot_waveforms( figdata.spikes, figdata.handles.id(which), 3 );
...
function init_panels( h )
    ...
    for k = 1:length(figdata.clus_list)
        ...
        a = axes('Parent',p);   
        plot_waveforms( figdata.spikes, figdata.clus_list(k), 3 );
