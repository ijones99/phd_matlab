...
% waitbar(0, max(floor(num_trials/100),1), ['Extracting Spikes . . . Thresh: ', num2str(thresh,2)] )
for j = 1:num_trials
    
    if ~isfield(spikes.params.display,'progressbar') || spikes.params.display.progressbar == 1
    ...
    % ijones added last_read_ts
    % this is so that spike crossings are added in the right position
    % compared to previous chunks.
    
    if isfield(spikes, 'last_read_ts')
        spikes.spiketimes   =  [spikes.spiketimes (spikes.last_read_ts + crossings)/ params.Fs];
    else
...
function c = get_covs( data, samples )
    ...
    for j = 1:max_samples
       %% added by ijones 
%        if sum((data_index(j)+[0:samples-1]) < 1) > 0
%            waves(j,:,:) = data{tr_index(j)}(data_index(j)+[0:samples-1]+sum((data_index(j)+[0:samples-1]) < 1),:);  
%        else
