function [spikes assigns, choice] = test_ss_kmeans(spikes, options)
...
%     toc
    if rep/50 == round(rep/50)
       fprintf('Rep number %d. Time is %s\n', rep, datestr(now,0));
    end
end
...
cmap = jetm(numclusts);
spikes.info.kmeans.colors = cmap(randperm_ums(numclusts),:);
