...
stimNames = {'white_noise_checkerboard', 'Orig_Movie', 'Static_Median_Surround', ...
...
dirName.prof_wn = strcat(dirName.prof,'/white_noise_checkerboard/');
dirName.qds = strcat(dirName.figs, 'white_noise_checkerboard/quality_datasheets/');mkdir(dirName.qds)
%%
...
%     'save_footprint',1,'use_sep_dir', 'white_noise_checkerboard');

%% do init processing
ifunc.auto_run.initial_neuron_processing('flist_white_noise_checkerboard','White_Noise', [], [])
...
stimNames = {'White_Noise', 'White_Noise'}
...
for iNeur = 1:length(neurNames)%length(selNeuronInds)
    ...
    set(gcf, 'color', 'white')
    % load profile data
    data = load_profiles_file(currNeuronName,'use_sep_dir','white_noise_checkerboard')
    ...
    try % plot the sta data
        plot((data.white_noise_checkerboard.sta_temporal_plot.x_axis/acqFreq)*1e3, data.white_noise_checkerboard.sta_temporal_plot.staIms, ...
    ...
    [junk indSTAFrMin] = min(data.white_noise_checkerboard.sta_temporal_plot.staIms); indSTAFrMin = indSTAFrMin(1);
    [junk indSTAFrMax] = max(data.white_noise_checkerboard.sta_temporal_plot.staIms); indSTAFrMax = indSTAFrMax(1);
    % title
    ...
    spikeTimes = data.white_noise_checkerboard.spiketimes;
    subaxis(3,3,currSubplot , 'Holdaxis',0, ...
        'SpacingVertical',0.01,'SpacingHorizontal',0.01, ...
        'PaddingLeft',0.01,'PaddingRight',0.01,'PaddingTop',0.01,'PaddingBottom',0.01, ...
        'MarginLeft',0.12,'MarginRight',0.12,'MarginTop',.12,'MarginBottom',.12, ...
    ...
    mysort.plot.waveforms2D_2(data.white_noise_checkerboard.footprint_median, hdmea.MultiElectrode.electrodePositions,...
        'plotMedian', 1,...
        'maxWaveforms', 5000, ...
    ...
    subaxis(3,3,currSubplot , 'Holdaxis',0, ...
        ...
        'MarginLeft',0.12,'MarginRight',0.12,'MarginTop',.12,'MarginBottom',.12, ...
        'rows',[],'cols',[]);
    selIm = data.white_noise_checkerboard.sta(:,:,indSTAFrMin); selIm = (imrotate(flipud(selIm),-90));
    imagesc(selIm); axis square, axis off, hold on
    circle([data.white_noise_checkerboard.plot_info.imWidth/2 data.white_noise_checkerboard.plot_info.imWidth/2],(data.white_noise_checkerboard.plot_info.imWidth*data.white_noise_checkerboard.plot_info.apertureDiam/data.white_noise_checkerboard.plot_info.stimWidth)/2, ...
        data.white_noise_checkerboard.plot_info.imWidth,'k--',2);
    
    currSubplot = currSubplot+1;
    selIm = data.white_noise_checkerboard.sta(:,:,indSTAFrMax); selIm = (imrotate(flipud(selIm),-90));
    subaxis(3,3,currSubplot , 'Holdaxis',0, ...
        'SpacingVertical',0.01,'SpacingHorizontal',0.01, ...
        'PaddingLeft',0.01,'PaddingRight',0.01,'PaddingTop',0.01,'PaddingBottom',0.01, ...
        'MarginLeft',0.12,'MarginRight',0.12,'MarginTop',.12,'MarginBottom',.12, ...
        'rows',[],'cols',[]);
    imagesc(selIm); axis square, axis off, hold on
    circle([data.white_noise_checkerboard.plot_info.imWidth/2 data.white_noise_checkerboard.plot_info.imWidth/2],(data.white_noise_checkerboard.plot_info.imWidth*data.white_noise_checkerboard.plot_info.apertureDiam/data.white_noise_checkerboard.plot_info.stimWidth)/2,data.white_noise_checkerboard.plot_info.imWidth,'k--',2);
...
for i=1:length(neurNames)
    ...
    waveformFootprintData{i} = data.white_noise_checkerboard.footprint.averaged;
end
