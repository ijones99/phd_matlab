...
minNumSpikeForSTA = 120;
trackFileNo = [];
fileName = '30Apr2014_resultsForIan.mat';
rIdx = [1,1];
[tsMatrix uniqueClus] = spiketrains.extract_spiketrains_from_felix_sorter_results(...
    rIdx, fileName);
for    iFileNo = 1:length(tsMatrix)
    maxTimeSTACalcTime = iMaxTimeSTACalcTime;
   spikeTimes = tsMatrix{iFileNo}.ts;
    if length(spikeTimes)>minNumSpikeForSTA
    ...
    fileName = sprintf('%s_sta_%s_clus_%d', ...
        get_dir_date, suffixName(2:end),tsMatrix{iFileNo}.clus_num );
...
if strcmp(input('Save trackfileno file? [y/n] ','y'))
...
%     flistFileNames = filenames.get_filenames('*ntk','../proc');
    ...
    wnCheckBrdPosData{end}.fileName = strrep(strrep(fileNames(fileIdx).name,'st_',''),'.mat','');
