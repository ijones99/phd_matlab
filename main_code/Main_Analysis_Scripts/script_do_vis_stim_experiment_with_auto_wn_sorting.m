...
for iFileSel = 1:length(idxGoodFiles)
    ...
    profData{iFileSel}.clusNum = tsMatrixAuto(idxGoodFiles(iFileSel)).clus_num;
...
% idxSel = [];
% for i = idxGoodFiles
%     figure, imagesc(profData{i}.staIm);
%     choice = input('Good? >> ','s');
%     if strcmp(choice, 'g')
%         idxSel(end+1) = i
%     end
...
for i = 1:length(idxGoodFiles)
    [xFitPercent yFitPercent zInterp] = fit.find_rf_center_by_fitting_gaussian(...
        profData{i}.staImAdj,'mask_size_um',100);
    profData{i}.rfRelCtr = [round(xFitPercent*900/2) round(yFitPercent*900/2)]
    profData{i}.staImInterp=zInterp;

    fileName = sprintf('%d_RF_localization',profData{i}.clusNum);
    plotDir = sprintf('../Figs/Visual_Stim/%d/',profData{i}.clusNum);
    title(sprintf('Cluster %d Receptive Field', profData{i}.clusNum));
...
for i =1:length(idxGoodFiles)
    fprintf('%d) Cluster %d: move [%d %d]\n', i, profData{i}.clusNum,  ...
        profData{i}.rfRelCtr(1),profData{i}.rfRelCtr(2));

end

% profDataMat = cells.merge_cells(profData, profData(idxGoodFiles));
