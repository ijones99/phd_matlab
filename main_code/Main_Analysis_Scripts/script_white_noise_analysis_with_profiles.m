...
stimNames = {'White_Noise', 'Orig_Movie', 'Static_Median_Surround', 'Dynamic_Median_Surround', ...
    ...
    'Pixelated_Surround_50_Percent', 'Pixelated_Surround_90_Percent', 'Dots_of_Different_Sizes',...
    'Moving_Bars'};
...
for i=1
    ...
    [elNumbers ] = compare_els_between_dir_elconfiginfo(elConfigInfo,dirName.el ,'*.mat', flist);
    selElNumbers = elNumbers;
    pre_process_data(suffixName, flistName, 'config_type','overlapping', 'sel_by_els',selElNumbers);
    %% SUPERVISED SORTING (MANUAL)
    % create the cl_ files -> save in gp dir
    fileType = 'mat';
    ...
    binWidthMs =0.1;
    loadHeatmap = 0
    if and(exist(fullfile(dirName.gp,'heatMap.mat')),loadHeatmap)
        load(fullfile(dirName.gp,'heatMap.mat'))
    else
        [heatMap] = get_heatmap_of_matching_ts2(binWidthMs, tsMatrix  );
        save(fullfile(dirName.gp,'heatMap.mat'), 'heatMap')
    end
    ...
    if exist(fullfile(dirName.gp,'selNeuronInds.mat'))
        load(fullfile(dirName.gp,'selNeuronInds.mat'))
    else
        [selNeuronInds outputGroups] = find_unique_neurons2(tsMatrix, ...
            heatMap, 'matching_thresh',0.2 );
        save(fullfile(dirName.gp,'selNeuronInds.mat'), 'selNeuronInds')
    end
end
...
for iNeur = 1:length(neurNames)%length(selNeuronInds)
    ...
    % plot footprints
    [ppAmps elCoordsX elCoordsY] = plot_footprints_all_els(  elConfigInfo, data.White_Noise.footprint.averaged, ...
               'exclude_els', ...
        [1596 2001 2304 2505 2911  3214 3415 3820 4123 4324],'line_width', 1, 'scale', 3.5, 'plot_sel_els_red', ...
        data.White_Noise.inds_for_sorting_els);
...
dirName.sta_figs = '~/temp/ian_sorting_3'; %'../Figs/STA/';
