function out = DetectAxon(varargin)
...
%      'Chipaddress'     -    "slot" where chip is plugged [0-4]
%      'Stim_mode'       -    [previous==0; voltage==1; current==2]
%      'Phase'           -    [us; microseconds]
%      'StimNumberExp'   -    Number of stimulations in experiment sections
...
while i<=length(varargin)
    if not(isempty(varargin{i}))
        ...
        elseif strcmp(varargin{i}, 'Phase')
            i=i+1;
            Phase=varargin{i};
        elseif strcmp(varargin{i}, 'StimNumberExp')
            i=i+1;
...
cmd=(['BitStreamer -n -f ' out.config_name '.hex.nrk2']);
system(cmd)

%   Download to FPGA
cmd=(['nc 11.0.0.7 32124 < ' out.config_name '.cmdraw.nrk2']);
system(cmd);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Stimulate %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

for i                   =       1:length(out.chs_stim)
    
    disp(sprintf(['Stimulating electrode ' num2str(out.els_stim(i)) ' channel ' num2str(out.chs_stim(i)) ' now.\n']));
    ...
    enable=1; dacSel=0; cLarge=0; channel=out.chs_stim(i); broadcast=0;
    loop.push_connect_channel(Chipaddress, enable, dacSel, cLarge, out.chs_stim(i), broadcast); % first channel
    loop.flush();
    pause(3)
