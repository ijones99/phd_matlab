...
function SingleElExcitability(varargin)
...
for configs             =       1:length(stim_elcs);
    ...
    for Stim                =       1:StimNumberExp
        for i           	=       VoltN
            ...
            dacselection  	=       0;
            channel         =       stimChNo(1);
            loop.push_biphasic_pulse(Chipaddress, 0, channel, volt, Phase/50, epoch(i), round(delay*20), Stim_mode);
            loop.size()
            
            for i        =       1:1200
                loop.push_simple_delay(1);
            end
            
            for i        =       1:150
                loop.push_simple_delay(40);
            end
            loop.flush();
        end
    end
