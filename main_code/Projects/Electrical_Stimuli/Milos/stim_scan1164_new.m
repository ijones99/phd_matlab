...
%%  Date: 24 january 2013    Chip 1164     Stim elc:971   Stim voltage: 65mv  MOSR: R1=220 R2=220  v3 (close to electrolysis) 
% Step01
clear all; Info.Exptype='StimScan';
Info.Exptitle='try18';
Info.Parameter.StimElectrode=8267;
Info.FileName.Spike            = ['~/Recordings/16May2013/proc/' Info.Exptitle '.spike'];
Info.FileName.SpikeForm        = ['~/Recordings/16May2013/proc/' Info.Exptitle '.spikeform'];
Info.FileName.Map              = ['~/Recordings/16May2013/Configs/try18_neuromap.m'];
Info.FileName.Trig             = ['~/Recordings/16May2013/proc/' Info.Exptitle '.raw.trig'];
Info.FileName.Raw              = ['~/Recordings/16May2013/proc/' Info.Exptitle '.raw'];
Info.FileName.TrigRawAve       = ['~/Recordings/16May2013/proc/' Info.Exptitle '.averaged.traw'];
...
Info.Parameter.ConfigDuration=23; % [ms] length of triggered rec
Info.Parameter.TriggerLength=20*23; % [samples] length of triggered re
...
%Step02
 ...
 Info_Parameter_NumberStim = 60; % tells code to look at DAC channel to determine epoch changes (as opposed to counting num of stim)
 
%    system(['rm ' Info.FileName.TrigRawAve]);

%  cmd=['/home/bakkumd/Data/cpp/CMOS/bin/cmos_postprocessing    -o ' Info.FileName.TrigRawAve ' -r ' Info.FileName.Raw ' -s ' int2str(Info.Parameter.TriggerLength) ' -m ' Info.FileName.Map ' -c ' int2str(Info.Parameter.ConfigNumber) ' -n ' int2str(Info.Parameter.NumberStim)]
%  cmd=['/home/bakkumd/Data/cpp/CMOS/bin/cmos_postprocessing -a -o ' Info.FileName.TrigRawAve ' -r ' Info.FileName.Raw ' -s ' int2str(Info.Parameter.TriggerLength) ' -z ' int2str(Info.Parameter.TriggerTimeZero) ' -m ' Info.FileName.Map ' -c ' int2str(Info.Parameter.ConfigNumber) ' -n ' int2str(Info.Parameter.NumberStim)]
%  cmd=['/opt/cpp/CMOS/bin/cmos_postprocessing -h -a -o ' Info.FileName.TrigRawAve ' -r ' Info.FileName.Raw ' -s ' int2str(Info.Parameter.TriggerLength) ' -z ' int2str(Info.Parameter.TriggerTimeZero) ' -m ' Info.FileName.Map ' -c ' int2str(Info.Parameter.ConfigNumber) ' -n ' int2str(Info.Parameter.NumberStim)]
...
%  cmd=['`which cmos_postprocessing` -a -o ' Info.FileName.TrigRawAve ' -r ' Info.FileName.Raw ' -s ' int2str(Info.Parameter.TriggerLength) ' -z ' int2str(Info.Parameter.TriggerTimeZero) ' -m ' Info.FileName.Map ' -c ' int2str(Info.Parameter.ConfigNumber) ' -n ' int2str(Info_Parameter_NumberStim)]
%  cmd=['/opt/cpp/CMOS/bin/cmos_postprocessing -a -N -o ' Info.FileName.TrigRawAve ' -r ' Info.FileName.Raw ' -s ' int2str(Info.Parameter.TriggerLength) ' -z ' int2str(Info.Parameter.TriggerTimeZero) ' -m ' Info.FileName.Map ' -c ' int2str(Info.Parameter.ConfigNumber) ' -n ' int2str(Info.Parameter.NumberStim)]

% %  LOW RES
% %  cmd=['/home/bakkumd/Documents/Code/CMOS/bin/cmos_postprocessing -a -L -o ' Info.FileName.TrigRawAve ' -r ' Info.FileName.Raw ' -s ' int2str(Info.Parameter.TriggerLength) ' -z ' int2str(Info.Parameter.TriggerTimeZero) ' -m ' Info.FileName.Map ' -c ' int2str(Info.Parameter.ConfigNumber) ' -n ' int2str(Info_Parameter_NumberStim)]
...
while i<=size(mtraw,2) 
 %  for i = 99
    ...
    if FILL      
        ...
        %scatter(ELC.X(ok),ELC.Y(ok),40,tar,'filled'); set(gca,'YDir','reverse')

 
       % z=griddata(ELC.X(ok),ELC.Y(ok),min( target(ok,max([34 i-40]):i)'),sx,sy,'cubic');              % binned to see trajectories of axons
       % z=griddata(ELC.X(ok),ELC.Y(ok),min( target(ok,max([100 i-110]):i)'),sx,sy,'cubic');              % binned to see trajectories of axons
       ...
       imagesc(sx(1,:),sy(:,1),z);   axis([sx(1,[1 end]) sy([1 end],1)'])
        ...
        %    z=griddata(ELC.X(ok),ELC.Y(ok),min( target(ok,(Info.Parameter.TriggerTimeZero+10):end)' ),sx,sy,'cubic');  % overall evoked
...
% % plotelectrodenumbers %   % plotelectrodenumbers %  % plotelectrodenumbers %  % plotelectrodenumbers %  % plotelectrodenumbers %  % plotelectrodenumbers %  % plotelectrodenumbers %  
% a=axis;
...
% % plotelectrodenumbers %  % plotelectrodenumbers %  % plotelectrodenumbers %  % plotelectrodenumbers %  % plotelectrodenumbers %  % plotelectrodenumbers %  % plotelectrodenumbers %     
   ...
   if SAVE
    ...
    filename=sprintf('/home/ijones/Recordings/16May2013/analysed_data/movie/pics/%s%s_%03d',Info.Exptitle,desc,cnt);
    cnt=cnt+1;
...
% cmd=['ffmpeg -y -i ' filename '_%03d.jpg -pass 1 -vcodec libx264 -vpre fastfirstpass -s vga -b 512k -bt 512k -threads 0 -f mp4 -an /dev/null && ffmpeg -i ' filename '_%03d.jpg -pass 2 -vcodec libx264 -vpre hq -b 512k -bt 512k -threads 0 -f mp4 ' filename '.mp4']
   ...
   filename_i = ['/home/ijones/Recordings/16May2013/analysed_data/movie/pics/' Info.Exptitle desc '_']
   filename_o = ['/home/ijones/Recordings/16May2013/analysed_data/movie/' Info.Exptitle desc '_']
