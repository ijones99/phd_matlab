function stim_bars_ds_scan( w,Settings,screenRect, repNum )
...
try
    ...
    currDirections = Settings.DIRECTIONS(repNum,:);
    barHeightPx = Settings.BAR_HEIGHT_PX(repNum);
    movingBarSpeedPx = Settings.SPEED(repNum)/Settings.UM_TO_PIX_CONV;
    rgbBrightness = Settings.RGB(repNum);
    
    for iAngle = currDirections
        for iHeight = barHeightPx
            for iVel = movingBarSpeedPx
                for iBrightness = rgbBrightness
                        ...
                        grCycleWidthPx = iHeight; % width of cycle in um
