...
for iStimGp = 1:length(fileNameConfig)
    ...
    settings.amps2{iStimGp} = [500 -500 0];
...
for iFile=1:length(fileNameConfig)
    % Convert hex to cmdraw.nrk2fileNameConfig
    iFile;
...
save(sprintf('flist_%s.mat', sortingRunName ), 'flist');
...
for i=1:length(flist)
    try
    spikeTimesSamples =stimPulsePts{i};
    h5Data = mea1{i};
    [data] = extract_waveforms_from_h5(h5Data, spikeTimesSamples );
    rmsAvgWaveform = rms(data.average,2);
    p2pAvgWaveform = max(data.average,[],2) - min(data.average,[],2);
    snrAvgWaveform = p2pAvgWaveform./rmsAvgWaveform;
    ...
    configElNumbers{i} = mea1{i}.MultiElectrode.electrodeNumbers;
...
%% compile movie
movie.simpleMovie( mposx, mposy, mat' , 'CAxis' , [-15 2] , 'Directory', 'test_stim');
