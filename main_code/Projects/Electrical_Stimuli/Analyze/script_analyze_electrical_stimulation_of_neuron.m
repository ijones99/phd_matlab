...
stimChNo = out.configs_stim_volt.stimCh{fileInd};
ampIndex = 1:size(out.configs_stim_volt.amps2{1},1);
...
out.configs_stim_volt.amps2{fileInd}(ampIndex,1);
...
for iChNo = 1:length(readOutChNo)
    ...
    if iChNo == 1
        ...
        text(xAxisVector(70), dacPulse(end)+80,  ...
            sprintf('pk2pk: %d mV',out.configs_stim_volt.amps2{1}(j+1,1)*2),'FontSize',11);
    end
...
title(sprintf('Regional Scan #1, Biphasic Stimulation. Stim Ch %d; %3.0f mV (DAC write), %3.0f setting (DAC read), %3.0f setting (DAC write)', ...
    stimChNo, mVPerDacVal*out.configs_stim_volt.amps(j), dacValues(j),out.configs_stim_volt.amps(j)),'FontSize',fontSize);
...
if doPrintToFile
    fileName = sprintf('%s_%s_Response_Traces_StimCh_%d_%03.0f_mV_%dDAC_read_Val_%dDAC_write',sortingRunName,...
        pulseType, stimChNo, mVPerDacVal*dacValues(j),dacValues(j),out.configs_stim_volt.amps(j));
    dirName = '../Figs/';
    saveas(h,fullfile(dirName, fileName),'fig');
    savefigtofile(h, dirName, fileName)
end
