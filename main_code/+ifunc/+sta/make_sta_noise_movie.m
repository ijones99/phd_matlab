function [staFrames staTemporalPlot plotInfo ] = ...
    make_sta_noise_movie( spikeTimes, dirNameStimFrames, frameInd, varargin)
...
neuronName='';
flistFileNameID = '';
dirSTA = strcat('../Figs/responses/Noise_Movie/');
doPrint = 0;
fontSize = 12;
apertureDiam = 500;
stimWidth = 1200;
totalNumFrames=max(frameInd); breakInterval=1000; numDelFrames = 26;
...
% ---- init sta array ---- %
stimNames = dir((fullfile(dirNameStimFrames,sprintf('frame_%08d.mat', 500))));
load(fullfile(dirNameStimFrames,stimNames(1).name));
staFrames = uint8(zeros(size(frame,1), size(frame,2),length(staAveragingTimesRelToSpike_samples)));
...
% rowsCols
fileInd = ifunc.file.create_file_index_for_frame_files_with_incontinuous_numbers(...
    totalNumFrames, breakInterval, numDelFrames);

for i=1:length(staAveragingTimesRelToSpike_samples) % cycle through all the timepoints around the spike occurrence
    ...
    fileIndsToAvg = fileInd(frameIndsToAvg);
    selFrames = single(zeros(size(frame,1), size(frame,2)));
    for j=1:length(fileIndsToAvg)
        nextFrame = ifunc.movie.get_specific_movie_frames(fileIndsToAvg(j), dirNameStimFrames);
        if ~isempty(nextFrame)
            selFrames = single(single(nextFrame)+ single(selFrames));
        else
            fprintf('Error %d\n', j)
        end
    end
    staFrames(:,:,i) = selFrames/length(fileIndsToAvg);
    fprintf('%d\n',i);
end
