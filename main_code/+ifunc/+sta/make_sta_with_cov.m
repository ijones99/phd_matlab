function [staFrames staTemporalPlot plotInfo bestSTA h] = ...
    make_sta_with_cov( spikeTimes, stimFrames, frameInd, varargin)
...
for i=1:length(staAveragingTimesRelToSpike_samples); % cycle through alclearl the timepoints around the spike occurrence
    ...
    stimFrameEdgeL = size(stimFrames,1);
    numFrameIdx = length(frameIndsToAvg);
    
    matReshaped = reshape(stimFrames(:,:,frameIndsToAvg),[stimFrameEdgeL^2 numFrameIdx]);
%     covMat = cov(matReshaped);
    %     covMatResh = reshape(covMat,[stimFrameEdgeL stimFrameEdgeL numFrameIdx]);
    [pCoeff, score] = princomp(single(matReshaped)');
    frame = reshape(pCoeff(:,1),[12 12]);
