...
spikesorting.batch_preprocess_files
...
% spikesorting.spt.spikesort_data('flistName', sprintf('flist%s_n_%02d',...
%     stimNames{stimNamesIdx}, runNo), patches );
spikesorting.spt.spikesort_data_flist_select

%% extract timestamps for spots

spikesorting.extract_spiketimes_from_cl_files()
...
stimNames = { ...
    '_wn_checkerboard_n_';...
    '_marching_sqr_and_moving_bars_n_'; ...
    '_spots_n_'; ...
};
...
suffixName = sprintf('%s%02d',stimNames{selStimName},runNo);
% make flist
flistName = sprintf('flist%s',suffixName);
...
ctrElidx = 6387;
flistName = 'flist_wn_checkerboard_n_02';
flist = {}; eval(flistName);
review_clusters(flistName,'center_elidx', ctrElidx,'add_dir_suffix','open_only')

flistName = 'flist_marching_sqr_and_moving_bars_n_02';
flist = {}; eval(flistName);
review_clusters(flistName,'center_elidx', ctrElidx,'add_dir_suffix','open_only')

%% review clusters (spots)
flistName = 'flist_spots_n_02';
flist = {}; eval(flistName);
for i=1:length(flist)
    fprintf('(%d) %s\n', i, flist{i});
end
selFlist = input('Select flist above >> ');

review_clusters(flistName,'center_elidx', ctrElidx,...
'add_dir_suffix','open_only','sel_flist',selFlist )

%% set all names
runNo = input('Enter run number >>'); 
neur = {};

neur.wn_checkerbrd.clusterName =    '6387n55';
neurFileName = sprintf('run_%02d_cell_%s', runNo, neur.wn_checkerbrd.clusterName);
neur.msmb.clusterName =             '6387n140';
[neur.msmb.elidxCtr  neur.msmb.clusNo] = filenames.parse_cluster_name(neur.msmb.clusterName);
neur.ms.clusterName =               '6387n140';
[neur.ms.elidxCtr  neur.ms.clusNo] = filenames.parse_cluster_name(neur.ms.clusterName);
neur.mb.clusterName =               '6387n140';
[neur.mb.elidxCtr  neur.mb.clusNo] = filenames.parse_cluster_name(neur.mb.clusterName);
neur.spt.clusterName =              '6387n20';
...
flistNo = input('flist number for spots ');
