...
loadingChunkSize = 2000;
waveformData = [];
for i=1:ceil(length(waveformInds)/loadingChunkSize)
    idxStart = (i-1)*loadingChunkSize+1;
    idxStop = (i)*loadingChunkSize;
    if idxStop > length(waveformInds)
        idxStop = length(waveformInds);
    end
    waveformDataCurr = h5Data.getData(waveformInds(idxStart:idxStop),:);
    waveformData = [waveformData;waveformDataCurr];
end
