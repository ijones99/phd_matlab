function [cross,lags,collision_info] = plot_xcorr_use_inds( spikes1, spikes2, show1, show2 )
...
%
   ...
   % check parameters
   if ~isfield(spikes1,'waveforms'), error('No waveforms found in spikes object.'); end
   select1 = show1;
   select2 = show2;

    % set internal data
    data.st1 = spikes1.unwrapped_times(select1);
    data.st2 = spikes2.unwrapped_times(select2);
    data.show_xcorr = spikes1.params.display.default_xcorr_mode;
    data.shadow             = spikes1.params.shadow;
    data.refractory_period  = spikes1.params.refractory_period;
    data.corr_bin_size           = spikes1.params.display.correlations_bin_size;
    data.maxlag = spikes1.params.display.max_autocorr_to_display;  % (msec);
    ...
    data.exp_rpv = round( 2*((data.refractory_period - data.shadow)/1000)*length(data.st1)*length(data.st2)/sum(spikes1.info.detect.dur) );
