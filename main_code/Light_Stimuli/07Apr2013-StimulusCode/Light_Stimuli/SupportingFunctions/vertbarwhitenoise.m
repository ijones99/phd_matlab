...
Screen(window, 'FillRect', [0 GrayVal GrayVal 0]);
Screen(window, 'PutImage', [0 GrayVal GrayVal 0]);
Screen(window, 'Flip');
...
for i = 1:NumFrames 
  ...
  clear expandedframe1
  expandedframe1(:,:,2) =   (kron(saved_frames(:,:,i),ones(NumPixelsPerSquare)));
  expandedframe1(:,:,3) = expandedframe1(:,:,2);
  expandedframe1(find(expandedframe1==0)) = DarkGrayVal;
  expandedframe1(find(expandedframe1==1)) = LightGrayVal;
  expandedframe1(:,:,1) = expandedframe1(:,:,1)*0;
  
  w = Screen(window, 'MakeTexture',  expandedframe1 );
