function drift_pattern_basic(angle, cyclespersecond, f, drawmask, gratingsize)
% function DriftDemo2([angle=30][, cyclespersecond=1][, f=0.05][, gratingsize=400])
...
% make gray window for intermissions
edgeLengthH = 600; edgeLengthV = 600;    
...
% 	screenNumber=max(screens);
	...
	% color to gray:
% 	[w screenRect]=Screen('OpenWindow',screenNumber, [0 200 200] );
    ...
    cosPart = cos(fr*x);;
    cosPart(find(cosPart > 0)) = 1;
    cosPart(find(cosPart <= 0)) = 0;
    
    grating=gray + inc*cosPart;
    ...
    while(vbl < vblendtime)
        ...
        Screen('DrawTexture', w, gratingtex, srcRect, dstRect, angle, [], [], [0 255 255], [], 1 );
