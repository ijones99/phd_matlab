function show_letter( edgeLength, doManual)
...
if fullScreen
    ...
    m(:,:,3)=m(:,:,2);
    w = zeros(1,2);
    letterA = imread('~/Matlab/During_Experiment/Letter_Images/Texas Letter-A.jpg');
    letterA = double(letterA./max(max(max(letterA))));
    letterA = imresize(letterA,[edgeLengthH,edgeLengthV]);
