function stim_09_wn_100per_checkerboard( w,Settings,StimMats, varargin )
brightSqrValDeci  = [];
if ~isempty(varargin)
...
try
    ...
    visiblesize=Settings.SURR_DIMS(1);
    ...
    if ~isempty(brightSqrValDeci)
        expanded_frame1 = ones(Settings.SURR_DIMS)*brightSqrValDeci;
    else
        expanded_frame1 = ones(Settings.SURR_DIMS)*1;
    end
    LEFT_TOP_CORNER = round((Settings.SURR_DIMS(1) - Settings.WN_EDGE_LENGTH(1))/2)+1;%left and top corner
    ...of stimulus
    ...
    transW = Screen('MakeTexture', w,  double(Settings.BLACK_VAL)+...
        double(Settings.MID_GRAY_VAL)*double(zeros(Settings.SURR_DIMS)));
