function stim_slits( w,Settings,StimMats,screenRect )
...
try
    ...
    %     grating = StimMats.slitImage(:,round(end*.25):round(end*.75))*Settings.MID_GRAY_VAL;
    ...
    transMatrix = Settings.MID_GRAY_VAL*ones(Settings.SURR_DIMS,Settings.SURR_DIMS  );
    transMatrixTex=Screen('MakeTexture', w, transMatrix);
    
    for iHeight = Settings.SLIT_HEIGHT_PX
        for iVel = Settings.SLIT_DRIFT_VEL_PX

            for iBrightness = Settings.SLIT_BRIGHTNESS;
                for iRepeats = 1:Settings.SLIT_REPEATS
                    % Animationloop:
                    ...
                    Settings.grDriftSpeedPxSec = iVel;
                    Settings.grCyclesPerSecond = Settings.grDriftSpeedPxSec/Settings.grCycleWidthPx  ;

                    % make slit image to show on screen
                    horScrDim = Settings.SURR_DIMS(1); vertScrDim = Settings.SURR_DIMS(2);
                    StimMats.slitImage = uint8(ones(vertScrDim, horScrDim*2 ));
                    StimMats.slitImage( round(vertScrDim/2)-round(iHeight/2):...
                        round(vertScrDim/2)+round(iHeight/2),...
                        round(horScrDim):...
                        end)=iBrightness*Settings.WHITE_VAL;

                    grating = StimMats.slitImage*Settings.MID_GRAY_VAL;
                    %     grating(:,round(end/2):end)=255;
                    ...
                    while(i < Settings.SURR_DIMS(1)/shiftperframe)%vblendtime)
                        i
                        ...
                        Screen('DrawTexture', w, gratingtex, srcRect, dstRect, Settings.angle);
                    ...
                    Screen('DrawTexture', w, transMatrixTex);
                    vbl = Screen('Flip', w);
                    pause(Settings.SLIT_PAUSE_BTWN_REPEATS);
