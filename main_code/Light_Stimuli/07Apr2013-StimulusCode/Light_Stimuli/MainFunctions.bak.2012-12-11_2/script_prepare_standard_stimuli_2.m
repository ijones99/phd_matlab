...
for i=1:1
    ...
    Settings.UM_TO_PIX_CONV = 1/0.6144;
    surroundDimsUm = 1200;
    % BRIGHTNESS VALUES
    pixTransferFuncDir = '/MainFunctions/';
...
%     load(fullfile(pixTransferFuncDir, 'polyFitCoeff.mat'));
    ...
    % SURROUND STIM SPECS
    Settings.SURR_DIMS = round(([surroundDimsUm surroundDimsUm])/Settings.UM_TO_PIX_CONV); %height and width of surround; ...
...
for stimType = 1
    ...
    apply_projector_transfer_function_nearest_neighbor(255*1/8,lookupTbMean_ND2_norm);  % -50 percent above middle gray   
        ];
    Settings.BAR_HEIGHT_PX = [ 100 300 600  900]/Settings.UM_TO_PIX_CONV;
    Settings.BAR_WIDTH_PX = 300/Settings.UM_TO_PIX_CONV;
    
    Settings.BAR_DRIFT_ANGLE = [0:7]*360;
    
    numEvents = (Settings.BAR_REPEATS*length(Settings.BAR_DRIFT_VEL_PX)*length(Settings.BAR_DRIFT_ANGLE)*...
        length(Settings.BAR_BRIGHTNESS)*length(Settings.BAR_HEIGHT_PX)*length(Settings.BAR_DRIFT_VEL_PX));
...
for stimType = 1
    ...
    Settings.WN_STIM_RES_PX = round(Settings.WN_STIM_RES_UM/Settings.UM_TO_PIX_CONV); % Resolution of surround stim in pix
     ...
     Settings.CHECKER_COUNT_DIMS = round([Settings.SURR_DIMS]/Settings.WN_STIM_RES_UM);
    
    Settings.N_FRAMES = Settings.STIM_DUR * Settings.STIM_FREQ * 60;
