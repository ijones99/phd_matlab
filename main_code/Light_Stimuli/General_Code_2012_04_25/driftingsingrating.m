function driftingsingrating(angle, cyclespersecond, freq, gratingsize, internalRotation, presentationLength, phase)
% derived from function DriftDemo4([angle=0][, cyclespersecond=1][, freq=1/360][, gratingsize=360][, internalRotation=0][, presentationLength=60] [, phase=0])
% ___________________________________________________________________
% presentationLength = time in seconds for presentation
% phase in degrees
...
% Presentation length of stimulus (unless interrupted)
if nargin < 7 || isempty(presentationLength)
    presentationLength = 60; % seconds
end
...
win = Screen('OpenWindow', screenid, [0 grayVal grayVal]);
...
for j=1:2
    ...
    % create and dray gray screen at the end of each run
    m=zeros(400,400, 3);
    m(:,:,2)=ones(400);
    m(:,:,3)=m(:,:,2);
