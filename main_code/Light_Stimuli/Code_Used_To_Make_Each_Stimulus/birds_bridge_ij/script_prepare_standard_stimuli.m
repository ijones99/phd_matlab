...
for i=1:1
    ...
    Settings.UM_TO_PIX_CONV = 1.79;

    % BRIGHTNESS VALUES
    Settings.BLACK_VAL = 0;
    Settings.WHITE_VAL = 255;
    Settings.DARK_GRAY_DECI = 115/Settings.WHITE_VAL;
    Settings.MID_GRAY_DECI = 163/Settings.WHITE_VAL;
    Settings.LIGHT_GRAY_DECI = 205/Settings.WHITE_VAL;
    ...
    % SURROUND STIM SPECS
    Settings.SURR_DIMS = round(([1074 1074])/Settings.UM_TO_PIX_CONV); %height and width of surround; ...
...
Settings.STIM_DUR = 60*15; % Duration of stimulus (seconds)*
Settings.SCR_DELAY = 0.0167;
Settings.INTER_FRAME_PAUSE_CAL = (1/Settings.STIM_FREQ) - Settings.SCR_DELAY;
Settings.BEGIN_EXP_PAUSE = 5;% seconds to wait before beginning
Settings.END_EXP_PAUSE = 5;% seconds to wait at end
Settings.POST_REPS_SERIES_PAUSE = 2; % Pause after series of repetitions
...
for stimType = 1
    ...
    Settings.BAR_REPEATS = 3;
    Settings.BAR_PAUSE_BTWN_REPEATS = 5;
    Settings.BAR_DRIFT_VEL_PX = [200 600 1200]/Settings.UM_TO_PIX_CONV;
    %     Settings.BAR_BRIGHTNESS = [Settings.WHITE_VAL Settings.BLACK_VAL ];
    Settings.BAR_BRIGHTNESS = [163-60 163-40 163-20 163+60 163+40 163+20 ];
    Settings.BAR_HEIGHT_PX = [ 100 300 600  900]/Settings.UM_TO_PIX_CONV;
    Settings.BAR_WIDTH_PX = 300/Settings.UM_TO_PIX_CONV;
    Settings.barDriftSpeedPx = Settings.BAR_DRIFT_VEL_PX;

    runTime = (Settings.BAR_REPEATS*Settings.BAR_PAUSE_BTWN_REPEATS*length(Settings.BAR_DRIFT_VEL_PX)*...
        length(Settings.BAR_BRIGHTNESS)*length(Settings.BAR_HEIGHT_PX)*length(Settings.barDriftSpeedPx)*800/200)/60; %minutes
...
for stimType = 1
    ...
    % All in seconds
    Settings.DOT_STIM_REPEATS = 5;
    Settings.DOT_INTER_SUB_SESSION_WAIT = 5;
    Settings.DOT_DOT_SHOW_TIME = 2;
    ...
    StimMats.varDotStimMtx = (false(Settings.SURR_DIMS(1), Settings.SURR_DIMS(2), 1));
...
for stimType = 1
    ...
    white_noise_frames = false(Settings.CHECKER_COUNT_DIMS(1), Settings.CHECKER_COUNT_DIMS(2), COUNT_FRAMES);

    rand('state',255)
    for k = 1:45*60*Settings.STIM_FREQ
