...
% Go 2 - 75
close all
for iCount =1:length(finalCluster)
    
    try
        iii=iCount;
        %         for iii=1:length(finalCluster)
        % plot data for each cluster
        cluster=iii;
        
        % plot in columns
        idx_light=(find(diff(double(ntk2.images.frameno))==1)+860);
        ts_onoff=finalCluster{cluster}.ts;
        ...
        ISIstamps = ((finalCluster{i}.ts(2:end) - finalCluster{i}.ts(1:end-1)))/20; %ms: calculate ISIs for each neuron cluster
        ...
        stdTrace = [];
        [Imax,Ymax] = max(finalCluster{i}.template.data,[],1);
        [Imin,Ymin] = min(finalCluster{i}.template.data,[],1);
        ...
        for j=size(finalCluster{i}.trace{Y}.data,1)
            stdTrace(end+1)=std(finalCluster{i}.trace{Y}.data(j,:));
        end
        ...
        plot_neurons(finalCluster{i},'elidx','dotraces_gray','electrodes', [finalCluster{i}.el_idx(Y),finalCluster{i}.el_idx(Y2)])
        
        title(strcat(num2str(i),'//',num2str(i),'/',num2str(percentSubThreshold),'/',num2str(stdTrace),'/',finalCluster{i}.rgc_type))
        
        isi=finalCluster{i}.ts;
        isi=diff(isi/20);
        isi=isi(isi<50);
        x=0:0.5:50;
        ...
        if assignCellType
            ...
            if isempty(promptToSave ) || promptToSave == 'u'  || promptToSave == 'o' || promptToSave == 'f'|| promptToSave == 'b'|| promptToSave == 'O'|| promptToSave == 'F'
                ...
                %     finalCluster{iii}.trace = [];
                %save data to one cell
                %                 finalCluster{end+1} = finalCluster{iii};
                %save cell type
                switch promptToSave
                    case 'o'
                        disp('on');
                        finalCluster{iCount}.rgc_type = 'on';
                    case 'f'
                        disp('off');
                        finalCluster{iCount}.rgc_type = 'off';
                    case 'b'
                        disp('on-off');
                        finalCluster{iCount}.rgc_type = 'on-off';
                    case 'u'
                        disp('unidentified');
                        finalCluster{iCount}.rgc_type = 'unidentified';
                    case 'O'
                        disp('sustained_on');
                        finalCluster{iCount}.rgc_type = 'sustained_on';
                    case 'F'
                        disp('sustained_off');
                        finalCluster{iCount}.rgc_type = 'sustained_off';
                end
                
                finalCluster{iCount}.idx_light = idx_light;
                
                
            end
            
        end
        fprintf('Percent finished: %.0f\nNumber saved clusters: %d\n', iCount/length(electrode_list)*100, length(finalCluster));
        finalCluster
        
        % if  xx == 'y'
        %             disp('redo loop');
        %             %delete cells from last loop
        %             finalCluster(finalClusterLength+1:end)=[];
        %             %revert to prev loop
        %             iCount = iCount-1;
        % end
    catch
