function flashing_sqr( window, Settings)

% KbName('KeyNamesLinux') %run this command to get the
% names of the keys

fprintf('Start Flashing Square\n')
do_record = 1;
fullScreen=0;
showMouse = 0;
...
screenMatrixBlank{1}(find(screenMatrixBlank{1}==1)) = Settings.RGB_OFF_STIM_BKND;
...
%             end
            ...
            w = Screen(window, 'MakeTexture', screenMatrix{iRgbStimBrightness});
