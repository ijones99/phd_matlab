function stim_drifting_linear_pattern(window, Settings)
% inputLine, speedPxPerSec, movieDurationSecs
...
visiblesize=Settings.STIM_EDGE_SIZE_PX;        % Size of the grating image. Needs to be a power of two.
...
% each redraw:
shiftperframe= speedPxPerSec * waitduration;
...
for iAngle = 1:length(Settings.DIR)
    ...
    while(vbl < vblendtime)
        ...
        %Screen('DrawTexture', window, gratingtex, srcRect);
        
        Screen('DrawTexture', window, gratingtex, srcRect, [], Settings.DIR(iAngle));
